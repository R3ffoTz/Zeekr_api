...
class ZeekrClimate(CoordinatorEntity, ClimateEntity):
    ...
    _attr_preset_modes = ["standard", "quick_heat", "quick_cool"]
    ...
    def __init__(self, coordinator):
        ...
        self._attr_preset_mode = "standard"
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        if preset_mode == "quick_heat": 
            await self._send_climate_command(True, 28.5)
        elif preset_mode == "quick_cool": 
            await self._send_climate_command(True, 15.5)
        else: 
            await self.async_turn_on()
    ...
    async def _send_climate_command(self, active, temp=None):
        payload = {
            ...
            "setting": {
                "serviceParameters": [
                    {"key": "AC", "value": "true" if active else "false"}
                ]
            }
        }
        if active and temp:
            payload["setting"]["serviceParameters"].extend([
                {"key": "AC.temp", "value": str(temp)}, 
